 ...
 *
 * Validates both CRC fields and fills 'out_meta'.  On success,
 * out_meta->body_off and out_meta->body_len indicate the body slice
 * inside 'packet', ready to pass to osrx_sensor_unpack() or
 * osrx_parser functions without copying.
 *
 * Returns 1 on success (check crc*_ok for integrity), 0 if packet is
 * NULL, too short, or header is malformed.
 */
