 ...
 *
 * Body layout: "{aid}.{status}.{ts_b64}|{sid}>{state}.{unit}:{b62(scaled)}|"
 ...
 *
 * Integer-only display example (no float):
 ...
 *
 * Parses the raw packet and validates CRC, then unpacks the sensor field.
 * Returns 1 on full success; 0 on any error (including CRC mismatch).
 */
